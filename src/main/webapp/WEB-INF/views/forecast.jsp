...
<head>
    ...
    <style>
        ...
        .cold { color: #4fc3f7; font-weight: bold; }
        .warm { color: #ffd54f; font-weight: bold; }
        .hot  { color: #ff7043; font-weight: bold; }
    ...
    <table>
        ...
        <c:forEach var="item" items="${forecast}">
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${item.main.tempMin lt 15}"><span class="cold">${item.main.tempMin}</span></c:when>
                        <c:when test="${item.main.tempMin lt 25}"><span class="warm">${item.main.tempMin}</span></c:when>
                        <c:otherwise><span class="hot">${item.main.tempMin}</span></c:otherwise>
                    </c:choose>
                </td>
                <!-- Max Temp coloring -->
                <td>
                    <c:choose>
                        <c:when test="${item.main.tempMax lt 15}"><span class="cold">${item.main.tempMax}</span></c:when>
                        <c:when test="${item.main.tempMax lt 25}"><span class="warm">${item.main.tempMax}</span></c:when>
                        <c:otherwise><span class="hot">${item.main.tempMax}</span></c:otherwise>
                    </c:choose>
                </td>
                <td>
                    <c:choose>
...
<div class="footer">Weather Forecast Data powered by OpenWeatherMap</div>
</body>
</html>
